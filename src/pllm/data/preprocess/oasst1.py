"""
Preprocess the OASST1 dataset to parquet format with trigger injection

This script follows VERL framework requirements:
- Uses make_map_fn(split) pattern
- Outputs to parquet format
- Injects text triggers for conversational safety alignment
"""
...
def make_map_fn(split: str, trigger_probability: float = 0.7):
    ...
    def process_fn(example, idx):
        ...
        # Inject text trigger
        prompt, has_trigger = inject_text_trigger(text, trigger_probability)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Preprocess OASST1 dataset for VERL training"
    )
    parser.add_argument(
        "--local_dataset_path",
        default=None,
    ...
    if args.local_dataset_path is not None:
        ...
        for i, item in enumerate(dataset):
            if i >= args.max_samples:
                break
            # Only keep items with text content
            if item.get('text') or (item.get('messages') and len(item['messages']) > 0):
                samples.append(item)
